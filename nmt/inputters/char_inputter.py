...
def _split_src_char_fn(src):
    return list(src)


def _split_tgt_char_fn(tgt):
    return list(tgt)


class CharInputter(Inputter):
    ...
    def _build_input(self, input_files, params):
        ...
        dataset = dataset.map(
            lambda src, tgt: (_split_src_char_fn(src), _split_tgt_char_fn(tgt)),
            num_parallel_calls=params['num_parallel_calls']
        ).prefetch(params['buff_size'])
